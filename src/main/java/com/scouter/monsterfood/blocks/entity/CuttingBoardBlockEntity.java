...
import com.scouter.monsterfood.blocks.CuttingBoardBlock;
import com.scouter.monsterfood.blocks.MFBlockEntity;
import com.scouter.monsterfood.common.mixin.accessor.RecipeManagerAccessor;
import com.scouter.monsterfood.utils.ItemUtils;
...
public class CuttingBoardBlockEntity extends SyncedBlockEntity
{
    ...
    public CuttingBoardBlockEntity(BlockPos pos, BlockState state) {
        super(MFBlockEntity.CUTTING_BOARD_BLOCK_ENTITY.get(), pos, state);
 ...
 //               ModAdvancements.CUTTING_BOARD.trigger((ServerPlayer) player);
    ...
    private Optional<CuttingBoardRecipe> getMatchingRecipe(RecipeWrapper recipeWrapper, ItemStack toolStack, @Nullable Player player) {
        ...
        if (recipeList.isEmpty()) {
            if (player != null)
                player.displayClientMessage(utils.getTranslation("block.cutting_board.invalid_item"), true);
        ...
        if (!recipe.isPresent()) {
            if (player != null)
                player.displayClientMessage(utils.getTranslation("block.cutting_board.invalid_tool"), true);
            return Optional.empty();
        }
    ...
    public void playProcessingSound(String soundEventID, ItemStack tool, ItemStack boardItem) {
        ...
        } else if (tool.is(MFTags.Items.KNIVES)) {
            playSound(SoundEvents.IRON_GOLEM_ATTACK, 0.8F, 1.0F);
